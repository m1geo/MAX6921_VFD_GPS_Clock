...
int offset = 0;  // BST = +1 see calcLocalHour();
...
byte calcLocalHour(int year, byte month, byte day, byte hour)
{
  byte DoW = day_of_week(year, month, day); // 0 = Sunday, 6 = Saturday
...
void display_dst (byte a) {
  digits (16, 0, 0, 17, 0, 0, 13, 0, 0, 10, 0, 0, 10, 0, 0, (a % 10), 0, 0);
}
  ...
  while (Serial.available()) {
    if (gps.encode(Serial.read())) { // process gps messages
      ...
      if (age < 500) {
        ...
        if (calcLocalHour(Year, Month, Day, Hour) == true) {
          adjustTime(1 * SECS_PER_HOUR);
        }
      }
    }
  }
    ...
    if (now() != prevDisplay) { //update the display only if the time has changed
      if (second() == 46) { // show date
        ...
        for (i=0; i<FADER; i++) {
          display_bright(BRIGHT);
          delay(FADER-i);
          display_dst(calcLocalHour(year(), month(), day(), hour()));
          delay(i);
        }
